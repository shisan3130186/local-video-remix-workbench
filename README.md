...
当前代码版本：V0.3.4
当前状态：核心视频功能与“智剪 SmartCut”第一版品牌、首页和三类工作区均已通过用户人工验收。
...
下一步：进入用户测试包准备阶段，候选为内置精简版FFmpeg/FFprobe、首次启动检测、诊断日志和干净Windows安装测试；开始前由用户确认具体顺序。
...
NSIS 安装器已经可以成功生成，但当前只适合内部技术测试：安装包尚未内置 FFmpeg / FFprobe，也没有代码签名。AI/TTS密钥已支持在软件内使用Windows当前用户加密保存，环境变量只保留为开发备用。
