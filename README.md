...
当前代码版本：V0.3.4
当前状态：AI文案智能混剪MVP、前五个前端重构阶段和TTS第一阶段均已通过人工验收。
已完成：多视频导入、全部素材切片、片段预览图和时长、AI画面理解、文案逐句分镜、人工调整、VideoEngine导出、画布、平滑混剪、视频效果、画中画、本地BGM、导出结果列表，以及文案生成MP3配音和本地试听。
下一步：规划逐句配音与视频画面同步；当前配音尚未自动加入视频。
...
AI 能力：
当前通过火山引擎方舟完成预览图画面理解和文案逐句分镜匹配，并通过火山语音完成文案生成MP3和本地试听。ASR、OCR、自动字幕以及配音自动加入视频尚未实现。
