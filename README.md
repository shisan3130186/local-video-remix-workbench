...
当前代码版本：V0.5.0
当前状态：V0.5.0国内账号会员系统和独立可视化卡密管理器已于2026-07-24完成开发、国内公网回归、正式打包和用户确认；本轮提交本地Git，不推送GitHub。
...
第四阶段新增：安装包内置FFmpeg/FFprobe、四步首次启动向导、AI/TTS/ASR配置检测、启动与崩溃日志、无密钥诊断报告、隐私说明和第三方许可证。正式安装版直接作为Windows桌面程序启动，不附带黑色终端窗口。
V0.5.0新增：邮箱注册登录、兑换码充值、个人中心会员状态、修改密码退出其他设备、同账号换机恢复、ECDSA服务端签名、72小时离线宽限，以及管理员本机可视化卡密管理器。
下一步：优先完成独立干净Windows主链路与卸载回归；之后再由用户确认代码签名、自动更新和正式用户协议的顺序。
...
双击项目根目录`启动智剪卡密管理器.vbs`。管理器支持预设和自定义会员天数、生成并复制、状态筛选、停用和恢复；它只供管理员本机使用，不进入普通用户安装包。
