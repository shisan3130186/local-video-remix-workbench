...
- 第四阶段V0.4.0用户测试包已于2026-07-21完成人工验收：内置FFmpeg/FFprobe、四步首次启动向导、AI/TTS/ASR配置检测、启动/崩溃日志、诊断报告、隐私与许可证说明；115.84MB安装包可独立运行，正式版不附带终端窗口。
- V0.4.1已于2026-07-24完成人工验收：所有后台FFmpeg/FFprobe调用统一隐藏控制台窗口，解决导入、切片和导出过程中反复闪黑窗。
- V0.5.0国内账号会员系统已于2026-07-24完成：邮箱注册登录、兑换码充值、个人中心、服务端签名、Windows安全缓存、CloudBase国内接口和独立可视化卡密管理器均可用。
- 最近稳定基线已包含水印工具、SmartCut第一版UI、V0.5.0国内账号会员系统和可视化卡密管理器；本地Git保存，暂不推送GitHub。
...
8. 已完成并验收：内置FFmpeg、首次启动检测、诊断信息和普通Windows用户测试包。
9. 已完成：会员远程验证、个人中心和可视化卡密管理器。
10. 下一阶段优先候选为独立干净Windows主链路与卸载回归；之后再做代码签名、自动更新和正式用户协议。
...
- 用户测试包向导：`src/features/user-test-package`
- 诊断与崩溃日志：`src-tauri/src/diagnostics.rs`
- 安装包验证：`scripts/verify-user-test-package.ps1`
- 会员服务：`services/membership-api`
- 可视化卡密管理器：项目根目录`启动智剪卡密管理器.vbs`
